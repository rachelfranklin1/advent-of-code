...
function plotWireCoordinates() {
  const positionTrailRight = {
    x: 0,
    y: 0
  };
  const positionTrailLeft = {
    x: 0,
    y: 0
  };
  ...
  firstPath.forEach(item => {
    plotter(item, positionTrailRight, firstCoordinates);
  });
  secondPath.forEach(item => {
    plotter(item, positionTrailLeft, secondCoordinates);
  });
